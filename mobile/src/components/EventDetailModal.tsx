...
export const EVENT_DATABASE_MAP: Record<string, EventDetailData> = {
  'sound-of-downtown': {
...
export function EventDetailModal({ eventId, onClose, onSuccessOrder }: EventDetailModalProps) {
  ...
  // If no eventId, do not render content inside Modal
  const isVisible = !!eventId;
  const eventKey = eventId && EVENT_DATABASE_MAP[eventId] ? eventId : 'sound-of-downtown';
  const event = EVENT_DATABASE_MAP[eventKey];

  const activeTierId = selectedTierId || event.tiers[0]?.id || 'vip';
  const selectedTier = event.tiers.find((t) => t.id === activeTierId) || event.tiers[0];
  const totalPrice = selectedTier ? selectedTier.price * ticketQuantity : 0;
  ...
  const handleConfirmPayment = () => {
    ...
    const msg = `Pembelian ${ticketQuantity}x ${selectedTier.name} (${event.title}) sebesar ${
      totalPrice === 0 ? 'GRATIS' : `Rp ${totalPrice.toLocaleString('id-ID')}`
    } BERHASIL! Tiket langsung dapat diakses di tab "Tiket Saya".`;
    ...
    if (typeof window !== 'undefined' && window.alert) {
      ...
      Alert.alert('Transaksi Berhasil!', msg, [
        { text: 'OK', onPress: () => onSuccessOrder?.() },
      ]);
    }
  };
        ...
        <View style={styles.container}>
          ...
          <View style={styles.topBar}>
            <Pressable onPress={onClose} style={styles.circleBtn}>
              <MaterialIcons name="close" size={22} color="#0b1a3d" />
            </Pressable>
            ...
            <View style={styles.topRightGroup}>
              ...
              <Pressable
                onPress={() => {
                  if (typeof window !== 'undefined' && window.alert) {
                    window.alert(`Link event ${event.title} disalin!`);
                  }
                }}
            ...
            <View style={styles.mainContent}>
              ...
              <View style={styles.quantityRow}>
                ...
                <View style={styles.counterBox}>
                  ...
                  <ThemedText type="headlineMd" style={{ color: theme.text, paddingHorizontal: 14 }}>
                    {ticketQuantity}
                  </ThemedText>
              ...
              <View style={styles.modalSheet} onStartShouldSetResponder={() => true}>
                ...
                <Pressable onPress={handleConfirmPayment} style={styles.confirmPayBtn}>
                  <ThemedText type="headlineMd" style={{ color: '#FFFFFF' }}>
                    Bayar {totalPrice === 0 ? 'Gratis' : `Rp ${totalPrice.toLocaleString('id-ID')}`}
                  </ThemedText>
                </Pressable>
              </View>
            </Pressable>
